...
public struct MachOImage {
    ...
    public init(target: DockTarget) throws {
        ...
        var found: [String: (UInt64, UInt64)] = [:]
            ...
            for _ in 0..<ncmds {
                ...
                if cmd == UInt32(LC_SEGMENT_64) {
                    ...
                    for s in 0..<nsects {
                        ...
                        let seg = MachOImage.name(raw, so + 16)
                        let addr = raw.loadUnaligned(fromByteOffset: so + 32, as: UInt64.self)
                        let size = raw.loadUnaligned(fromByteOffset: so + 40, as: UInt64.self)
                        found["\(seg),\(sect)"] = (addr, size)
                    }
                }
                offset += cmdsize
            }
        ...
        guard let linked = textVMAddr else { throw SpaceSwitchError.imageNotFound }
        let slide = base &- linked
        sections = found.mapValues { (address: $0.0 &+ slide, size: $0.1) }
    }
    ...
    public var text: (address: UInt64, size: UInt64) {
        get throws {
            guard let t = sections["__TEXT,__text"] else { throw SpaceSwitchError.imageNotFound }
            return t
        }
    }
}
